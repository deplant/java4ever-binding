...
public final class WildcardTypeName extends TypeName {
	...
	private WildcardTypeName(List<TypeName> upperBounds, List<TypeName> lowerBounds,
		...
		for (TypeName upperBound : this.upperBounds) {
			checkArgument(!upperBound.isPrimitive() && upperBound != TypeName.VOID,
			              "invalid upper bound: %s", upperBound);
		}
		for (TypeName lowerBound : this.lowerBounds) {
			checkArgument(!lowerBound.isPrimitive() && lowerBound != TypeName.VOID,
	...
	public static WildcardTypeName supertypeOf(TypeName lowerBound) {
		return new WildcardTypeName(Collections.singletonList(TypeName.OBJECT),
	...
	CodeWriter emit(CodeWriter out) throws IOException {
		...
		return this.upperBounds.get(0).equals(TypeName.OBJECT)
				? out.emit("?")
